...
subgroups = [[i, j] for i in range(2) for j in range(2)]
...
for model_file in model_files:
    ...
    y_pred = np.array([1 if item>=0.5 else 0 for item in model.predict(X_test.reshape(-1, 1, 6)).flatten()])
...
with open('results/compas/prune_multiple_attributes.json', "w") as file:
