...
for model_file in model_files:
    ...
    y_pred = np.array([1 if item>=0.5 else 0 for item in model.predict(X_test.reshape(-1, 1, 20)).flatten()])
...
with open('results/GC/prune_multiple_attributes.json', "w") as file:
