...
dataset_test = StandardDataset(X_test_df, label_name='credit',
                 favorable_classes=[1],
                 protected_attribute_names=['age'],
                 privileged_classes=[[1]],
                 instance_weights_name=None,
                 categorical_features=['status', 'credit_history', 'purpose', 'savings', 'employment', 'other_debtors', 'property', 'installment_plans', 
                'housing', 'skill_level', 'telephone'],
                 na_values=['?'], custom_preprocessing=None)

dataset_train = StandardDataset(X_train_df, label_name='credit',
                 favorable_classes=[1],
                 protected_attribute_names=['age'],
                 privileged_classes=[[1]],
                 instance_weights_name=None,
...
for model_file in model_files:
    ...
    res = model.predict(X_train.reshape(1, -1, 20)).flatten()
    ...
    res = model.predict(X_test.reshape(1, -1, 20)).flatten()
    y_pred = [[1] if s>=0.5 else [0] for s in res]
...
with open('results/GC/CalibratedEqOdds.json', "w") as file:
