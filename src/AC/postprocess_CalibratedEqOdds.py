...
dataset_test = StandardDataset(X_test_df, label_name='income-per-year',
                 favorable_classes=[1, 1],
                 protected_attribute_names=['race', 'sex'],
                 privileged_classes=[[1], [1]],
                 instance_weights_name=None,
                 categorical_features=['age', 'workclass', 'education',
                     'marital-status', 'occupation', 'relationship',
                     'native-country'],
                 features_to_keep=[], features_to_drop=['fnlwgt'],
                 na_values=['?'], custom_preprocessing=None)

dataset_train = StandardDataset(X_train_df, label_name='income-per-year',
                 favorable_classes=[1, 1],
                 protected_attribute_names=['race', 'sex'],
                 privileged_classes=[[1], [1]],
                 instance_weights_name=None,
...
for model_file in model_files:
    ...
    res = model.predict(X_train.reshape(1, -1, 13)).flatten()
    ...
    res = model.predict(X_test.reshape(1, -1, 13)).flatten()
    y_pred = [[1] if s>=0.5 else [0] for s in res]
...
with open('results/AC/CalibratedEqOdds.json', "w") as file:
