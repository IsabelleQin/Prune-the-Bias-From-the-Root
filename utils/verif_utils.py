...
def load_compas():
    ...
    # print(X)
    # print(y)
    print(len([i for i in range(len(y.values)) if X['Race'][i] == 0]))
    print(len([i for i in range(len(y.values)) if X['Race'][i] == 1]))
    exit()
    ...
    # this last line exists only inside Fairify
    # int allowed for X here because the data is all int
    return (df, X_train.to_numpy().astype('int'), y_train.to_numpy().astype('int'), X_val.to_numpy().astype('int'), y_val.to_numpy().astype('int'), X_test.to_numpy().astype('int'), y_test.to_numpy().astype('int'))
